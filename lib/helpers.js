...
function addNullTypes(swagger) {
  return traverse(swagger).forEach(function(node) {
    if (this.key !== 'type') return;
    
    if (typeof node === 'string') this.update([ node, 'null' ]);
    else if (Array.isArray(node) && node.indexOf('null') === -1) {
      this.update(node.concat([ 'null' ]));
    }
  });
}
